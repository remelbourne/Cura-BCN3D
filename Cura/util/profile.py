...
# New options that we are creating for our own version of simple mode, these are for when only using one extruder
setting('quality_fast', False, bool, 'single', _('Quality')).setLabel(_("Fast Print   "))
setting('quality_standard', False, bool, 'single', _('Quality')).setLabel(_("Standard Quality   "))
setting('quality_high', False, bool, 'single', _('Quality')).setLabel(_("High Quality   "))
setting('quality_strong', False, bool, 'single', _('Quality')).setLabel(_("Strong Quality   "))

setting('extruder_left', False, bool, 'single', _('Extruder')).setLabel(_("Left Extruder   "))
setting('extruder_right', False, bool, 'single', _('Extruder')).setLabel(_("Right Extruder   "))

setting('material_pla', False, bool, 'single', _('Material')).setLabel(_("PLA   "))
setting('material_abs', False, bool, 'single', _('Material')).setLabel(_("ABS   "))
setting('material_fila', False, bool, 'single', _('Material')).setLabel(_("FilaFlex   "))

# New options that we are creating for our own version of simple mode, these are for when we are using double extruder
setting('quality_fast_dual', False, bool, 'dual', _('Quality')).setLabel(_("Fast Print   "))
setting('quality_standard_dual', False, bool, 'dual', _('Quality')).setLabel(_("Standard Quality   "))
setting('quality_high_dual', False, bool, 'dual', _('Quality')).setLabel(_("High Quality   "))
setting('quality_strong_dual', False, bool, 'dual', _('Quality')).setLabel(_("Strong Quality   "))

setting('pla_left_dual', False, bool, 'dual', _('Left Material')).setLabel(_("PLA   "))
setting('abs_left_dual', False, bool, 'dual', _('Left Material')).setLabel(_("ABS   "))
setting('fila_left_dual', False, bool, 'dual', _('Left Material')).setLabel(_("FilaFlex   "))

setting('pla_right_dual', False, bool, 'dual', _('Right Material')).setLabel(_("PLA   "))
setting('abs_right_dual', False, bool, 'dual', _('Right Material')).setLabel(_("ABS   "))
setting('fila_right_dual', False, bool, 'dual', _('Right Material')).setLabel(_("FilaFlex   "))
setting('pva_right_dual', False, bool, 'dual', _('Right Material')).setLabel(_("PVA   "))

setting('dual_support', False, bool, 'dual', _('Right Material')).setLabel(_("Need supports ?   "))

#End of simple mode for only BCN3D Technologies
...
setting('print_bed_temperature',      45, int,   'basic',    _('Speed and Temperature')).setRange(0,340).setLabel(_("Bed temperature (C)"), _("Temperature used for the heated printer bed. Set at 0 to pre-heat yourself."))
...
setting('filament_diameter',        2.88, float, 'basic',    _('Filament')).setRange(1).setLabel(_("Diameter (mm)"), _("Diameter of your filament, as accurately as possible.\nIf you cannot measure this value you will have to calibrate it, a higher number means less extrusion, a smaller number generates more extrusion."))
...
setting('travel_speed',            150.0, float, 'advanced', _('Speed')).setRange(0.1).setLabel(_("Travel speed (mm/s)"), _("Speed at which travel moves are done, a well built BCN3D Technologies printer can reach speeds of 250mm/s. But some machines might miss steps then."))
...
""", str, 'alteration', 'alteration')
#######################################################################################
setting('endleft.gcode', """;End GCode
...
""", str, 'alteration', 'alteration')
#######################################################################################
setting('endright.gcode', """;End GCode
...
#setting('lastFirmwareFile', os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'resources', 'example', 'MarlinBCN3DSigma.hex')), str, 'preference', 'hidden')
...
setting('model_colour', '#118DCD', str, 'preference', 'hidden').setLabel(_('Model colour'), _('Display color for first extruder'))
...
def getAlterationFileContents(filename, extruderCount):
    ...
    if getMachineSetting('gcode_flavor') == 'UltiGCode':
        ...
        if filename == 'endleft.gcode':
            return 'M25 ;Stop reading from this point on.\n;CURA_PROFILE_STRING:%s\n' % (getProfileString())
        if filename == 'endright.gcode':
            return 'M25 ;Stop reading from this point on.\n;CURA_PROFILE_STRING:%s\n' % (getProfileString())
        return ''
    if filename == 'start.gcode':
        #Really messy way of doing this, but hey at least it works!
        if getPreference('startMode') == 'Simple' and getMachineSetting('machine_type') == 'BCN3DSigma':
            #All the options for when we choose FAST PRINT
            if getProfileSetting('quality_fast') == 'True':
                #Options for the left side and all the possible materials
                if getProfileSetting('extruder_left') == 'True' and getProfileSetting('material_pla') == 'True':
                    prefix += 'T0\n'
                    prefix += 'M190 S45\n'
                    prefix += 'M109 T0 S220\n'
                    prefix += 'M104 T1 S25\n'
                elif getProfileSetting('extruder_left') == 'True' and getProfileSetting('material_abs') == 'True':
                    prefix += 'T0\n'
                    prefix += 'M190 S70\n'
                    prefix += 'M109 T0 S250\n'
                    prefix += 'M104 T1 S25\n'
                elif getProfileSetting('extruder_left') == 'True' and getProfileSetting('material_fila') == 'True':
                    prefix += 'T0\n'
                    prefix += 'M190 S100\n'
                    prefix += 'M109 T0 S250\n'
                    prefix += 'M104 T1 S25\n'
                #Options for the right side and all the possible materials
                elif getProfileSetting('extruder_right') == 'True' and getProfileSetting('material_pla') == 'True':
                    prefix += 'T1\n'
                    prefix += 'M190 S45\n'
                    prefix += 'M109 T1 S220\n'
                    prefix += 'M104 T0 S25\n'
                elif getProfileSetting('extruder_right') == 'True' and getProfileSetting('material_abs') == 'True':
                    prefix += 'T1\n'
                    prefix += 'M190 S70\n'
                    prefix += 'M109 T1 S250\n'
                    prefix += 'M104 T0 S25\n'
                elif getProfileSetting('extruder_right') == 'True' and getProfileSetting('material_fila') == 'True':
                    prefix += 'T1\n'
                    prefix += 'M190 S100\n'
                    prefix += 'M109 T1 S250\n'
                    prefix += 'M104 T0 S25\n'
                putProfileSetting('layer_height', '0.3')
                putProfileSetting('wall_thickness', '0.8')
                putProfileSetting('fill_density', '15')
                putProfileSetting('bottom_layer_speed', '60')
                putProfileSetting('infill_speed', '120')
                putProfileSetting('solidarea_speed', '80')
                putProfileSetting('inset0_speed', '60')
                putProfileSetting('insetx_speed', '80')
            #All the options for when we choose STANDARD PRINT
            elif getProfileSetting('quality_standard') == 'True':
                #Options for the left side and all the possible materials
                if getProfileSetting('extruder_left') == 'True' and getProfileSetting('material_pla') == 'True':
                    prefix += 'T0\n'
                    prefix += 'M190 S45\n'
                ...
                #Options for the right side and all the possible materials
                    ...
                    prefix += 'M109 T1 S250\n'
                    prefix += 'M104 T0 S25\n'
                putProfileSetting('layer_height', '0.2')
                putProfileSetting('wall_thickness', '0.8')
                putProfileSetting('fill_density', '15')
                putProfileSetting('bottom_layer_speed', '40')
                putProfileSetting('infill_speed', '60')
                putProfileSetting('solidarea_speed', '40')
                putProfileSetting('inset0_speed', '40')
                putProfileSetting('insetx_speed', '50')
            #All the options for when we choose HIGH QUALITY PRINT
            elif getProfileSetting('quality_high') == 'True':
                #Options for the left side and all the possible materials
                if getProfileSetting('extruder_left') == 'True' and getProfileSetting('material_pla') == 'True':
                    prefix += 'T0\n'
                    prefix += 'M190 S45\n'
                ...
                #Options for the right side and all the possible materials
                    ...
                    prefix += 'M109 T1 S250\n'
                    prefix += 'M104 T0 S25\n'
                putProfileSetting('layer_height', '0.1')
                putProfileSetting('wall_thickness', '0.8')
                putProfileSetting('fill_density', '15')
                putProfileSetting('bottom_layer_speed', '35')
                putProfileSetting('infill_speed', '50')
                putProfileSetting('solidarea_speed', '35')
                putProfileSetting('inset0_speed', '30')
                putProfileSetting('insetx_speed', '40')
            #All the options for when we choose SRONG QUALITY PRINT
            elif getProfileSetting('quality_strong') == 'True':
                #Options for the left side and all the possible materials
                if getProfileSetting('extruder_left') == 'True' and getProfileSetting('material_pla') == 'True':
                    prefix += 'T0\n'
                    prefix += 'M190 S45\n'
                ...
                #Options for the right side and all the possible materials
                    ...
                    prefix += 'M104 T0 S25\n'
                putProfileSetting('layer_height', '0.2')
                putProfileSetting('wall_thickness', '1.6')
                putProfileSetting('fill_density', '30')
                putProfileSetting('bottom_layer_speed', '35')
                putProfileSetting('infill_speed', '60')
                putProfileSetting('solidarea_speed', '35')
                putProfileSetting('inset0_speed', '30')
                putProfileSetting('insetx_speed', '45')
            #If we choose to print with idex and fast
            elif getProfileSetting('quality_fast_dual') == 'True':
                #If we are in fast and we want both sides to print with pla
                if getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('pla_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        ...
                        prefix += 'M109 T0 S220\n'
                        prefix += 'T0\n'
                elif getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('abs_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S220\n'
                        prefix += 'T0\n'
                elif getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('fila_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S220\n'
                        prefix += 'T0\n'
                elif getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('pva_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S220\n'
                        prefix += 'T0\n'
                elif getProfileSetting('abs_left_dual') == 'True' and getProfileSetting('pla_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('abs_left_dual') == 'True' and getProfileSetting('abs_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('abs_left_dual') == 'True' and getProfileSetting('fila_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('abs_left_dual') == 'True' and getProfileSetting('pva_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('fila_left_dual') == 'True' and getProfileSetting('pla_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('fila_left_dual') == 'True' and getProfileSetting('abs_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('fila_left_dual') == 'True' and getProfileSetting('fila_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                        ...
                        prefix += 'M109 T0 S250\n'
                        prefix += 'T0\n'
                elif getProfileSetting('fila_left_dual') == 'True' and getProfileSetting('pva_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                        eSteps = getMachineSettingFloat('steps_per_e')
                ...
                putProfileSetting('inset0_speed', '60')
                putProfileSetting('insetx_speed', '80')
            elif getProfileSetting('quality_standard_dual') == 'True':
                #If we are in standard and we want both sides to print with pla
                if getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('pla_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                ...
                putProfileSetting('inset0_speed', '40')
                putProfileSetting('insetx_speed', '50')
            elif getProfileSetting('quality_high_dual') == 'True':
                #If we are in high and we want both sides to print with pla
                if getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('pla_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
                ...
                putProfileSetting('inset0_speed', '30')
                putProfileSetting('insetx_speed', '40')
            elif getProfileSetting('quality_strong_dual') == 'True':
                if getProfileSetting('pla_left_dual') == 'True' and getProfileSetting('pla_right_dual') == 'True':
                    if getProfileSetting('dual_support') == 'True':
                        putProfileSetting('dual_support', 'True')
                        getAlterationFile("start2.gcode")
            ...
            alterationContents = getAlterationFile("start.gcode")
    elif filename == 'end.gcode':
        if getPreference('startMode') == 'Simple' and getMachineSetting('machine_type') == 'BCN3DSigma' and getProfileSetting('extruder_left') == 'True':
            alterationContents = getAlterationFile("endleft.gcode")
        elif getPreference('startMode') == 'Simple' and getMachineSetting('machine_type') == 'BCN3DSigma' and getProfileSetting('extruder_right') == 'True':
            alterationContents = getAlterationFile("endright.gcode")
        elif getPreference('startMode') == 'Simple' and getMachineSetting('machine_type') == 'BCN3DSigma':
            alterationContents = getAlterationFile("end2.gcode")
